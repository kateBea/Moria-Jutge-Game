#include "Player.hh"
//PLAYER PRETTY SCUFFED

/**
 * Write the name of your player and save this file
 * with the same name and .cc extension.
 */
#define PLAYER_NAME Kate
...
struct PLAYER_NAME : public Player 
{
  ...
  /*Possible targets to go for*/
  enum TARGET {ENEMY_UNIT, TREASURE, FRIENDLY_UNIT, CAVE};
  ...
  Dir retrieve_dir(const std::map<Pos, Pos>& _vis, const Pos& fin, const Pos& start)
  {
    ...
    Dir ret = Dir::None;
    if (it->first == (it->second + Dir::Bottom))  ret = Dir::Bottom;
    if (it->first == (it->second + Dir::BR))      ret = Dir::BR;
    if (it->first == (it->second + Dir::LB))      ret = Dir::LB;
    if (it->first == (it->second + Dir::Left))    ret = Dir::Left;
    if (it->first == (it->second + Dir::Right))   ret = Dir::Right;
    if (it->first == (it->second + Dir::RT))      ret = Dir::RT;
    if (it->first == (it->second + Dir::TL))      ret = Dir::TL;
    if (it->first == (it->second + Dir::Top))     ret = Dir::Top;
    return ret;
  }

  /*return valid adjacent position that is in valid path to target*/
  //beard in mind stat s the position we start from (not first to enqueue)
    ...
    while (not next_on_line.empty() and not found)
    {
      ...
      for (int i = 0; i < (NATURAL_POS); ++i)
      {
        ...
        if (pos_ok(next_pos) and not pos_visited)
        {
          ...
          /*mark position has been visited*/
          ...
          if (cell_has_unit) temp = unit(current_cell.id);
          ...
          if (start_unit.type == UnitType::Wizard)
          {
            ...
            if (current_cell.type == CellType::Abyss) continue;
            if (current_cell.id == balrog_id()) continue;
            if (cell_has_unit and temp.type == UnitType::Troll) continue;
            if (cell_has_unit and temp.type == UnitType::Orc) continue;
            ...
            if (is_my_unit and temp.type == UnitType::Dwarf) 
            {
              ...
              {
                //std::cerr << "im wizard with id :" << start_unit.id << " value of i is: " << i << " we at round: " << round() << " adjacent position has my dwarf " << Pos(start_unit.pos+ret) << std::endl;
                ret = Dir::None;
              } 
              //std::cerr << "im wizard with id :" << start_unit.id << " and pos: " << start_unit.pos<<" value of i is: " << i << " we at round: " << round() << " i found my dwarf at pos: " << next_pos << " moving to "<< Pos(start_unit.pos+ret)<< std::endl;
              found = true;
              break;
            }
          }
          else 
          {
            ...
            bool not_my_unit = temp.player != me();                                     
            ...
            if (balrog or abyss or there_troll) continue;
            if (is_my_unit and temp.type == UnitType::Wizard) continue;

            /*find enemy dwarf nearby to engage*/
            bool engage = cell_has_unit and not_my_unit;
            if (engage)
            {
              ret = retrieve_dir(visited_pos, next_pos, start_unit.pos);
              found = true;
            ...
            /*cell is a cave not yet conquered*/
            else if (false and already_conquered)
            {
              ret = retrieve_dir(visited_pos, next_pos, start_unit.pos);
              found = true;
            }
            /*cell is a cave not yet conquered by another player*/
            else if (false and not_conquered)
            {
              ret = retrieve_dir(visited_pos, next_pos, start_unit.pos);
              found = true;
            }
  ...
  void move_wizards()
  {
    ...
    for (auto unit_id : my_wizards)
    {
      ...
      #if false
      bool unit_low_on_health = false;
      Pos x = temp.pos; x.i = 0;
      Pos y = temp.pos; y.j = 0;
      Cell current_cell1; //to loop by rows
      Cell current_cell2; //to loop by colums
      /*check horizontally and vertically for units low on health*/
      for (int i = 0; i < BOARD_SIZE and not unit_low_on_health; ++i)
      {
        current_cell1 = cell(x); //to loop by rows
        current_cell2 = cell(y); //to loop by colums
        bool my_unit1 = current_cell1.id != -1 and unit(current_cell1.id).player == me();
        bool my_unit2 = current_cell2.id != -1 and unit(current_cell2.id).player == me();
        bool dwarf_low_hp = temp.health < TRESHOLD and temp.type == UnitType::Dwarf;
        if ((my_unit1 or my_unit2) and dwarf_low_hp) unit_low_on_health = true;
        x.i = i;
        y.j = i;
      }
      /*stay to heal up unit, otherwise move to unit that needs heals*/
      if (not unit_low_on_health) 
      #endif
